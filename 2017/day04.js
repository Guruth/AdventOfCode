var testInput1 = "aa bb cc dd ee\n\
aa bb cc dd aa\n\
aa bb cc dd aaa";

var input1 = "nyot babgr babgr kqtu kqtu kzshonp ylyk psqk\n\
...
var validateDuplicate = function(passphrase){
	let result = true;
	...
	for(phrase of phraseArray){
		...
		let count = phraseMap.get(phrase) + 1;
		phraseMap.set(phrase,count);
	}

	for( [phrase, count] of phraseMap ){
		if(count !== 1){
			return false;
		}
	}
...
var validateAnagram = function(passphrase){
...
var test = function(expected, input, func){
...
var execTests = function(){
	var testInput = parseInput(testInput1);
	test(true, testInput[0], validateDuplicate);
	test(false, testInput[1], validateDuplicate);
	test(true, testInput[2], validateDuplicate);
}

execTests();

var runValidation = function(){
	var lines = parseInput(input1);
	var validCount = 0;
	for(let line of lines){
		if(validateDuplicate(line)){
			validCount++;
		}
	}
