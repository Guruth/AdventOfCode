...
var parseInput = function(input){
	...
	for(var char of input){
		if(char === "\n"){
			tmpArray.push(Number.parseInt(tmpNum));
			...
			tmpNum = tmpNum.concat(char);
		}
	}
	return result;
};

var sol1 = function(spreadsheet){
 	let sum = 0;
...
var test = function (expected,  input,  func){
	...
	if(result === expected){
		console.log("Result correct: " + expected + "===" + result);
	}else{
		console.log("Wrong: "+ expected +" but was "+ result);
	}
};

var execTest = function (){
	console.log("Test1");
	test(18, parseInput(testInput), sol1);
	
	console.log("Real Input");
	let result = sol1(parseInput(input));
	console.log(result);

	console.log("Test 2");
	test(9, parseInput(testInput2),sol2);

	console.log("Real Input 2");
	result = sol2(parseInput(input));
	console.log(result);
};
